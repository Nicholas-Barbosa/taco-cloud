...
class TestReactor {
	...
	@Test
	void testBufferFlux() {
		Flux<String> flux = Flux.just("data1", "data2", "data3", "data4");
		flux.buffer(2).log().subscribe(System.out::println);
		StepVerifier.create(flux.buffer(2)).expectNext(List.of("data1", "data2"), List.of("data3", "data4"))
				.expectComplete();
	}
	...
	void collectMap() {
		// Like Streams
		Flux<String> flux = Flux.just("data1", "data2", "data3", "data4");
		flux.collectMap(x -> "Key:" + x + "K", v -> "value: " + v).subscribe(System.out::println);
	}

	@Test
	void terminalLogicOperators() {
